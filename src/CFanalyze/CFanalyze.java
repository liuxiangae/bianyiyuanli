	...
	public void CFmain(String yourcode) {
		...
		try{
			...
			while((line=strcode.readLine())!=null){
				tools.outTextArea.append("Line "+count+" :"+"\n");
				...
				if(!ifNote){
					...
					if(begin>-1){
						if(end>-1){
							...
							tools.outTextArea.append("注释是："+note+"\n");
				...
				for(int i=0;i<strLine.length;i++){
					...
					if(isAlpha(ch)){
						...
						if(pos>=0 && pos!=14 && pos!=15){  //suggest it is key word 
							...
							tools.outTextArea.append(tmpToken+"   <"+pos+",--->"+"\n");
							...
							if(pos==14){
								...
								tools.outTextArea.append(tmpToken+"   <"+pos+",布尔常量 1>"+"\n");
								...
								tools.outTextArea.append(tmpToken+"   <"+pos+",布尔常量 0"+"\n");
							...
							//if(tablelist.isEmpty() || !isExist(tmpToken)){
								...
								tools.outTextArea.append(tmpToken+"    <1,"+(tablelist.size()-1)+">"+"\n");
						...
						if(isRightDig(tmpToken)){
							if(tmpToken.indexOf(".")==-1&&tmpToken.indexOf("e")==-1 ){
								...
								tools.outTextArea.append(tmpToken+"     <2,整数>"+"\n");
								...
								tools.outTextArea.append(tmpToken+"     <3,浮点数>"+"\n");
							...
							tools.outTextArea.append("ERROR: "+tmpToken+" 错误的数字输入！"+"\n");
						...
						if(RightChar){
							if(tmpToken.length()>1 ||tmpToken.length()==0){
								...
								tools.outTextArea.append(tmpToken+"     <4,字符常量>"+"\n");
							...
							tools.outTextArea.append("字符符号不封闭->"+tmpToken+"\n");
						...
						if(RightChars){
							...
							tools.outTextArea.append("\""+tmpToken+"\""+"     <5,字符串常量>"+"\n");
							...
							tools.outTextArea.append("字符串不封闭->"+tmpToken+"\n");
						...
						if(ch!='/' && ch!='*'){//suggest it is divide /
							if(ch=='='){//  /=
								...
								tools.outTextArea.append(tmpToken+"     <"+symbolrecord.isOper2(tmpToken)+",--->"+"\n");
								...
								tools.outTextArea.append(tmpToken+"     <"+symbolrecord.isOper('/')+",--->"+"\n");
						...
						if((ch=='+'||ch=='-'||ch=='*'||ch=='-'||ch=='='||ch=='<'||
							...
							tools.outTextArea.append(tmpToken+"      <"+symbolrecord.isOper2(tmpToken)+",操作码>"+"\n");
							...
							tools.outTextArea.append(tmpToken+"         <"+symbolrecord.isOper(ch)+",操作码>"+"\n");
							...
							if(!stack1.empty()){
								...
								tools.outTextArea.append("ERROR ：这个符号或之前的这个没有起始！ "+ch+"\n");
					...
					{  
						...
						{  
							...
							tools.outTextArea.append("%-10c ERROR:\n");
			...
			if(!stack1.empty()){
				while(!stack1.empty()){
					...
					tools.outTextArea.append("第"+stack1.size()+"个{符号没有结束符 "+top+"\n");
				}
			}
			if(!stack2.empty()){
				while(!stack2.empty()){
					String top=stack2.pop();
					tools.outTextArea.append("第"+stack2.size()+"个（符号没有结束符 "+top+"\n");
			...
			for(int nu=0;nu<tablelist.size();nu++){
				...
				tools.outTextArea.append(tablelist.get(nu).position+" "+tablelist.get(nu).sym_name+" "+tablelist.get(nu).type+"\n");
			}
